...
30 POKE 23659,0:PRINT AT 23,0;"Keys 5678  Space = Zoom ";m;"x";AT 0,0;:POKE 23659,2:LET p=(h-g)/31:LET q=(e-d)/21:IF m>2 THEN GOTO 10
...
60 LET o$=l$: LET l$=INKEY$:IF l$<>"" AND o$<>l$ THEN LET k=CODE l$:LET w=PEEK (z+f): LET w=((w>127)*(w-128))+(w<128)*w: POKE z+f,w: LET v=(k=56)-(k=53)+32*((k=54)-(k=55)):LET f=f+v: LET f=(f>=0 AND f<768)*f: LET w=PEEK(z+f):POKE z+f,w+128: IF l$=" " THEN LET m=m*2: LET p=(h-g)/4:LET q=(e-d)/4:LET y=INT(f/32):LET x=(f-(y*32)):LET x=g+x*(h-g)/32:LET x=g+(f-(y*32))*(h-g)/32:LET y=d+y*(e-d)/22:LET d=y-q:LET e=y+q:LET g=x-p:LET h=x+p: GOTO 30
